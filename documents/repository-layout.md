...
This repo is the **Code Coffee** [Slidev](https://sli.dev) deck for CODE LEAP AG. Nearly all slide copy and structure lives in a single Markdown file at the repo root: **`slides.md`**.

## Assets vs `public/` (prep vs what gets served)

| Location | Role |
|----------|------|
| **`assets/`** | **Content preparation** — source photos, exports, and other inputs *before* they are ready for the deck. Examples: year folders of individual portraits (`assets/individual_photos/...`), graphics you are still editing, small images **imported directly in Vue** (e.g. `assets/cert-logos/*.png` used by the certification layout). This is *not* where slide frontmatter URLs point for most full-bleed or avatar images. |
| **`public/`** | **What Slidev serves in the browser** — static files at the site root. Slide YAML uses paths **from `/`**: e.g. `avatar: /birthdays/jane-doe.jpg` → file `public/birthdays/jane-doe.jpg`. The template reads these URLs for photos, fonts, and large slide imagery. |
...
| Path | Role |
|------|------|
| **`slides.md`** | **Source of truth** for the deck — one `---`-delimited block per slide; YAML frontmatter picks `layout:` and passes props. |
| **`layouts/*.vue`** | Custom Vue slide layouts (cover, section, birthday, person, project, etc.). Extend when you need a new slide *structure*; routine copy stays in `slides.md`. |
| **`styles/*.css`** | Tokens, markdown bullets, per-layout styling. |
| **`style.css`** | Entry point that imports the files under `styles/`. |
